...
useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (packageData?.stayLocations?.length > 0) {
            const citiesFromStep1 = packageData.stayLocations.map(location => location.city);
            ...
            const updatedDestinationNights = citiesFromStep1.map(city => {
                const existingDest = tourDetails.destinationNights.find(dest => dest.destination === city);
                return existingDest || {
                    destination: city,
                    nights: packageData.stayLocations.find(loc => loc.city === city)?.nights || 0,
                    hotels: [
    ...
    const getHotelsForDestination = (destinationCity) => {
        ...
        console.log(`📊 Total hotels in filteredHotels: ${filteredHotels?.length || 0}`);
        ...
        const destinationHotels = filteredHotels.filter(hotel => {
            ...
            console.log(`🔸 Comparing: Hotel "${hotelName}" in "${hotelCity}" with "${searchCity}"`);
        ...
        console.log(`🏨 Found ${destinationHotels.length} hotels for "${destinationCity}"`);
    ...
    const getHotelOptionsForCategory = (category, destinationCity = "") => {
        ...
        console.log(`🏩 ${category} hotels for "${destinationCity}": ${baseOptions.length} from API + ${customOptions.length} custom = ${allOptions.length} total`);
    ...
    const handleAddNewItem = async () => {
        ...
        try {
            ...
            if (currentHotelCategory) {
                ...
                console.log(`➕ Adding new hotel: "${newValue}" for category "${currentHotelCategory}"`);
            }

            console.log(`📡 Calling API to add: ${newValue} to field: ${backendField}`);
    ...
    const handleHotelChange = (destIndex, category, hotelName) => {
        console.log(`🏨 Hotel change: Destination ${destIndex}, ${category}, ${hotelName}`);
    ...
    const handlePriceChange = (destIndex, category, price) => {
        ...
        setTourDetails({ ...tourDetails, destinationNights: updatedNights });
        console.log(`💰 Price updated: ${category} hotel to ₹${priceValue} for destination ${destIndex}`);
    };
            ...
            >
                ...
                {optData && (
                    <IconButton
                        ...
                        onClick={(e) => {
                            e.stopPropagation();
                            if (window.confirm(`Delete "${option}" from ${category} hotels?`)) {
                                dispatch(deleteLeadOption(optData._id));
                    ...
                    <TableBody>
                        {tourDetails.destinationNights.length > 0 ? (
                            tourDetails.destinationNights.map((dest, index) => {
                                ...
                                console.log(`🔄 Rendering destination: ${destinationCity} at index ${index}`);
                                ...
                                const totalPerPerson = (standardPrice || 0) + (deluxePrice || 0) + (superiorPrice || 0);
                                    ...
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>
                                            ...
                                            <TextField
                                                ...
                                                onChange={(e) => handlePriceChange(index, "standard", e.target.value)}
                                                fullWidth
                                                InputProps={{
                                        ...
                                        <TableCell>
                                            ...
                                            <TextField
                                                ...
                                                onChange={(e) => handlePriceChange(index, "deluxe", e.target.value)}
                                                fullWidth
                                                InputProps={{
                                                    startAdornment: <Typography variant="caption" sx={{ mr: 1 }}>₹</Typography>,
                                                }}
                                        ...
                                        <TableCell>
                                            ...
                                            <TextField
                                                ...
                                                onChange={(e) => handlePriceChange(index, "superior", e.target.value)}
                                                fullWidth
                                                InputProps={{
                                                    startAdornment: <Typography variant="caption" sx={{ mr: 1 }}>₹</Typography>,
                                                }}
