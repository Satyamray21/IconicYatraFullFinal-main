...
const TourDetailsForm = ({ clientName, sector, cities = [], onNext }) => {
  ...
  useEffect(() => {
    ...
    const lead = leadList.find((l) => {
      const nameMatch =
        ...
        l.personalDetails.fullName.trim().toLowerCase() ===
      ...
      const stateMatch =
        !!l?.location?.state &&
        l.location.state.trim().toLowerCase() === sectorLower;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      formData.append(
        "quotationId",
        window.localStorage.getItem("quotationId"),
      ); // <-- or from props
  ...
  useEffect(() => {
    ...
    const leadData =
      selectedLead.tourDetails?.pickupDrop || selectedLead.tourDetails || {};
    const arrivalDate = leadData.arrivalDate
      ? new Date(leadData.arrivalDate)
      : null;
    const departureDate = leadData.departureDate
    ...
    formik.setValues({
      ...formik.values,
      arrivalCity:
        leadData.arrivalCity ||
        leadData.arrivalLocation ||
        formik.values.arrivalCity ||
        "",
      departureCity:
        leadData.departureCity ||
        leadData.departureLocation ||
