...
const Home = () => {
  ...
  const { data, status } = useSelector((state) => state.homePage);
  useEffect(() => {
  ...
  const handleInquirySubmit = async (e) => {
    ...
    try {
      await enquiryAxios.post("/enquiry/create", data);
      navigate("/thank-you");
    } catch (error) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box position="relative">
        ...
        {/* ================= FIXED Inquiry Form (OUTSIDE MAP) ================= */}
        {showForm && (
          <Box
            sx={{
              position: "absolute",
...
<TrustedCompany data={data?.trustedAgency} />
