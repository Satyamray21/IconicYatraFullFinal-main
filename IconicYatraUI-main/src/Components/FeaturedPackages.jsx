...
import { fetchLatestPackages, clearError } from '../Features/packageSlice';
...
const FeaturedPackages = () => {
  ...
  // Fetch latest packages on component mount
  useEffect(() => {
  ...
  }, [cardsToShow, latestPackages.length]);
  ...
  const handleQueryClick = (pkg, e) => {
    e.stopPropagation();
    console.log("Query for latest package:", pkg);
  };
  ...
  // ENHANCED PRICE FUNCTION - SAME AS INTERNATIONAL PACKAGES
  ...
  // Check if price is available for conditional rendering
  const hasPrice = (pkg) => {
  ...
  return (
    <Box sx={{
      ...
      <Box textAlign="center" mb={3}>
        ...
        >
          LATEST <span style={{ color: "#FFD700" }}>PACKAGES</span>
        </Typography>
        <Typography variant="subtitle1" sx={{
          mb: 2
        }}>
          Discover our newest and most exciting travel experiences
        </Typography>
        <Divider
          sx={{
            borderColor: "#2196F3",
          ...
          >
            {latestPackages.map((pkg) => (
              ...
              >
                ...
                {/* UPDATED PRICE BADGE - SAME AS INTERNATIONAL */}
                {hasPrice(pkg) && (
                  <Box
                    sx={{
                      position: "absolute",
      ...
      {/* Navigation Arrows */}
      {latestPackages.length > cardsToShow && (
        <>
          <Button
            onClick={() => {
              if (scrollRef.current) {
